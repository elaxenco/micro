...
insert  into `corridas_tipo_c`(`id`,`cliente_id`,`desembolso_id`,`fecha_pago`,`pago_completo`,`capital`,`interes`,`pago_capital`,`pago_interes`,`estatus_id`,`saldo`) values (6,8,3,'2019-03-31','7700.00','7000.00','700.00','0.00','700.00',5,'0.00');
...
insert  into `desembolsos`(`id`,`fecha`,`primer_pago`,`capital`,`pago_completo`,`pago_capital`,`pago_interes`,`pago_seguro`,`estatus_id`,`tipo_id`,`cliente_id`,`capturista_id`,`fecha_registro`,`hora_registro`) values (1,'2019-03-20','2019-03-23','4500.00','535.00','322.00','193.00','20.00',5,1,5,1,'2019-03-20','20:14:48'),(3,'2019-03-20','2019-03-31','7000.00','7700.00','7000.00','700.00','0.00',5,3,8,1,'2019-03-20','20:09:41');
...
insert  into `importes`(`id`,`importe`,`interes`,`semanas`,`quincenas`,`pago_completo`,`pago_capital`,`pago_interes`,`seguro`,`total`,`totalSeguro`,`tipo_id`) values (1,'2500.00','1500.00',14,0,'306.00','179.00','107.00','20.00','4000.00','4280.00',1),(2,'3000.00','1800.00',14,0,'363.00','215.00','128.00','20.00','4800.00','5080.00',1),(3,'3500.00','2100.00',14,0,'420.00','250.00','150.00','20.00','5600.00','5880.00',1),(4,'4000.00','2400.00',14,0,'478.00','286.00','172.00','20.00','6400.00','6680.00',1),(5,'4500.00','2700.00',14,0,'535.00','322.00','193.00','20.00','7200.00','7480.00',1),(6,'5000.00','3000.00',14,0,'592.00','358.00','214.00','20.00','8000.00','8280.00',1),(7,'2500.00','1500.00',0,8,'540.00','360.00','140.00','40.00','4000.00','4320.00',2),(8,'3000.00','1800.00',0,8,'640.00','432.00','168.00','40.00','4800.00','5120.00',2),(9,'3500.00','2100.00',0,8,'740.00','504.00','196.00','40.00','5600.00','5920.00',2),(10,'4000.00','2400.00',0,8,'840.00','576.00','224.00','40.00','6400.00','6720.00',2),(11,'4500.00','2700.00',0,8,'940.00','648.00','252.00','40.00','7200.00','7520.00',2),(12,'5000.00','3000.00',0,8,'1040.00','720.00','280.00','40.00','8000.00','8320.00',2);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_estadodecuentacliente` AS (select `d`.`id` AS `desembolso_id`,`d`.`cliente_id` AS `cliente_id`,`d`.`capital` AS `capital`,`d`.`pago_completo` AS `pago_completo`,(case when (`d`.`tipo_id` < 3) then ifnull((select sum(`corridas`.`saldo`) AS `sum(``corridas``.``saldo``)` from `corridas` where ((`corridas`.`desembolso_id` = `d`.`id`) and (`corridas`.`fecha_pago` < curdate()))),0) when (`d`.`tipo_id` = 3) then ifnull((select `corridas_tipo_c`.`pago_completo` AS `pago_completo` from `corridas_tipo_c` where ((`corridas_tipo_c`.`desembolso_id` = `d`.`id`) and (`corridas_tipo_c`.`fecha_pago` <= curdate()) and (`corridas_tipo_c`.`estatus_id` = 5))),0) end) AS `saldoVencido`,(case when (`d`.`tipo_id` < 3) then ifnull((select sum(`corridas`.`saldo`) AS `sum(``corridas``.``saldo``)` from `corridas` where ((`corridas`.`desembolso_id` = `d`.`id`) and (`corridas`.`fecha_pago` <= curdate()))),0) when (`d`.`tipo_id` = 3) then ifnull((select `corridas_tipo_c`.`pago_completo` AS `pago_completo` from `corridas_tipo_c` where ((`corridas_tipo_c`.`desembolso_id` = `d`.`id`) and (`corridas_tipo_c`.`fecha_pago` <= curdate()) and (`corridas_tipo_c`.`estatus_id` = 5))),0) end) AS `pagoExigible`,(case when (`d`.`tipo_id` < 3) then ifnull((select sum(`corridas`.`saldo`) AS `sum(``corridas``.``saldo``)` from `corridas` where ((`corridas`.`desembolso_id` = `d`.`id`) and (`corridas`.`fecha_pago` <> 0))),0) when (`d`.`tipo_id` = 3) then ifnull((select `corridas_tipo_c`.`pago_completo` AS `pago_completo` from `corridas_tipo_c` where ((`corridas_tipo_c`.`desembolso_id` = `d`.`id`) and (`corridas_tipo_c`.`estatus_id` = 5))),0) end) AS `saldoTotal`,`d`.`tipo_id` AS `tipo_id` from `desembolsos` `d` where (`d`.`estatus_id` = 5)) */;
