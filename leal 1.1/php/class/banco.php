		...
		public function verCorteDeCaja($caja_id,$tipo_caja,$fecha){
				...
				if($corte=='S'){ 
						...
						while ($res = mysqli_fetch_row($resultado)){
							...
							$datos[$i]['saldo_inicial'] = $res[0]; 
							$datos[$i]['entradas'] 		= $res[1]; 
							$datos[$i]['capital'] 		= $res[2];    
							$datos[$i]['interes'] 		= $res[3];  
							$datos[$i]['seguro'] 		= $res[4]; 
							$datos[$i]['salidas'] 		= $res[5];   
							$datos[$i]['desembolsos'] 	= $res[6];  
							$datos[$i]['saldoFinal'] 	= $res[0]+$res[1]+$res[2]+$res[3]+$res[4]-$res[5]-$res[6];    
